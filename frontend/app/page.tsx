        ...
        .then((data) => {
          ...
          if (Array.isArray(items) && items.length > 0) {
             const mapped: PlaceCardProps[] = items.slice(0, 4).map((p: any, idx: number) => ({
               ...
               category: p.category?.name || initialPremierSpaces[idx % 4].category,
               sqm: p.capacity || initialPremierSpaces[idx % 4].sqm,
               ...
               price: Math.round(p.price) || initialPremierSpaces[idx % 4].price,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-12 sm:space-y-16">
      ...
      <section className="text-center space-y-6">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#FAF9F6] dark:bg-[#1A1E2F] border border-[#E7E5DE] dark:border-[#334155] text-[11px] font-bold tracking-wider uppercase text-[#777C78] dark:text-[#94A3B8]">
          <span className="w-1.5 h-1.5 rounded-full bg-[#063C2F] dark:bg-[#14B8A6]" />
          <span>Architectural Sanctuaries & Athletic Arenas</span>
        </div>

        <h1 className="text-3xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-[#111512] dark:text-[#F8FAFC] max-w-4xl mx-auto leading-[1.15]">
          Find the right space for every plan.
        </h1>

        <p className="text-sm sm:text-base text-[#555A56] dark:text-[#94A3B8] max-w-2xl mx-auto leading-relaxed">
        ...
        <div className="relative mt-8 rounded-3xl overflow-hidden border border-[#E7E5DE] dark:border-[#334155] shadow-xl aspect-[16/8] sm:aspect-[21/9] min-h-[320px]">
          <img
      ...
      <section className="py-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
          <div>
             <span className="text-[11px] font-bold tracking-writer uppercase text-[#777C78] dark:text-[#94A3B8]">
               Excellence in Craft
             </span>
